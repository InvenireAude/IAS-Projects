...
BEGIN
   ...
   std::send("output.status",ctx,data);


   IF routing.monRequest == "FULL" THEN BEGIN
	   esb::broker::interface::common::registerServiceCall(ctx.MID, "REQUEST", client, routing.provider, data, FALSE);
   END ELSE IF routing.monRequest == "HEADER" THEN BEGIN
	   esb::broker::interface::common::registerServiceCall(ctx.MID, "REQUEST", client, routing.provider, data, TRUE);
   END;
