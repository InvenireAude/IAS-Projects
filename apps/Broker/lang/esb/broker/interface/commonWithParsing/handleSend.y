  ...
  routing    = esb::broker::ds::fetchRouting(client, interface, service);
   ...
   std::send("output."+routing.provider,ctx,data);
    ...
    IF routing.monRequest == "FULL" THEN BEGIN
	   esb::broker::interface::common::registerServiceCall(ctx.MID, "DATAGRAM", interface, service, client, routing.provider, data, std::getTime(), (NULL AS DateTime), FALSE);
   END ELSE IF routing.monRequest == "HEADER" THEN BEGIN
	   esb::broker::interface::common::registerServiceCall(ctx.MID, "DATAGRAM", interface, service, client, routing.provider, data, std::getTime(), (NULL AS DateTime), TRUE);
   END;
